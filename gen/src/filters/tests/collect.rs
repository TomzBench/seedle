...
fn expect_collect() {
    ...
    let expect_literals = Value::Array(vec![Value::from(LinkedNode::Literal(Literal::UInt(3)))]);
    let expect_primatives = Value::Array(vec![Value::from(LinkedNode::Primative(
        ConstrainedPrimative::Bool,
    ))]);
    let expect_structs = Value::Array(vec![Value::from(LinkedNode::Struct(Fields {
        members: vec![
